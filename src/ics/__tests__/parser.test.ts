...
function makeTbaDeadline(overrides: { id: string; title: string; unit: string }): Deadline {
  return {
...
describe('matchIcsToDeadlines', () => {
  ...
  it('matches an ICS event to a TBA deadline by unit code', () => {
    const deadlines = [makeTbaDeadline({ id: '1', title: 'Final Examination', unit: 'COMP1005' })];
  ...
  it('gives low confidence when unit matches but no keyword overlap', () => {
    const deadlines = [makeTbaDeadline({ id: '1', title: 'Group Project', unit: 'COMP1005' })];
    const matches = matchIcsToDeadlines(events, deadlines);
    expect(matches).toHaveLength(1);
  ...
  it('does not match when unit codes differ', () => {
    const deadlines = [makeTbaDeadline({ id: '1', title: 'Final Exam', unit: 'ELEN1000' })];
    const matches = matchIcsToDeadlines(events, deadlines);
