...
describe('seriesKey', () => {
  it('strips trailing number from title so numbered items share a key', () => {
    expect(seriesKey('COMP1005', 'Practical Test 1')).toBe('COMP1005|Practical Test');
    expect(seriesKey('COMP1005', 'Practical Test 3')).toBe('COMP1005|Practical Test');
  });

  it('keeps the full title when there is no trailing number', () => {
    expect(seriesKey('COMP1005', 'Final Examination')).toBe('COMP1005|Final Examination');
...
function makeDeadline(overrides: Partial<Deadline> & { id: string; title: string; unit: string }): Deadline {
  return {
...
describe('buildDeadlineSections', () => {
  const upcoming1 = makeDeadline({ id: '1', title: 'Assignment', unit: 'COMP1005',
    dueDate: new Date(Date.now() + 3 * 86_400_000).toISOString() });
  const upcoming2 = makeDeadline({ id: '2', title: 'Lab Report', unit: 'MATH1019',
    dueDate: new Date(Date.now() + 5 * 86_400_000).toISOString() });
  const overdue1  = makeDeadline({ id: '3', title: 'Quiz', unit: 'COMP1005',
    dueDate: new Date(Date.now() - 2 * 86_400_000).toISOString() });
  const examTba   = makeDeadline({ id: '4', title: 'Final Examination', unit: 'COMP1005',
    dueDate: new Date(9999, 11, 31).toISOString(), dateTBA: true });
  const weekTba   = makeDeadline({ id: '5', title: 'Mid-Sem Test', unit: 'MATH1019',
    dueDate: new Date(9999, 11, 31).toISOString(), dateTBA: true, weekLabel: 'Week 7' });
  const fullTba   = makeDeadline({ id: '6', title: 'Online Quiz', unit: 'ELEN1000',
    dueDate: new Date(9999, 11, 31).toISOString(), dateTBA: true });

  const allDeadlines = [upcoming1, upcoming2, overdue1, examTba, weekTba, fullTba];
  const defaultOpts = { filterUnit: '', filterStatus: 'all' as const, sortBy: 'date' as const, overduePosition: 'bottom' as const };
  ...
  it('places overdue items before upcoming when overduePosition is "top"', () => {
    const { display } = buildDeadlineSections(allDeadlines, { ...defaultOpts, overduePosition: 'top' });
  ...
  it('filters by unit code', () => {
    const { display } = buildDeadlineSections(allDeadlines, { ...defaultOpts, filterUnit: 'COMP1005' });
    expect(display.every((d) => d.unit === 'COMP1005')).toBe(true);
  });

  it('filters to upcoming only (excludes TBA and overdue)', () => {
    const { display } = buildDeadlineSections(allDeadlines, { ...defaultOpts, filterStatus: 'upcoming' });
    expect(display.every((d) => !d.dateTBA && new Date(d.dueDate) > new Date())).toBe(true);
    expect(display.some((d) => d.dateTBA)).toBe(false);
    expect(display.some((d) => !d.dateTBA && new Date(d.dueDate) <= new Date())).toBe(false);
  });

  it('filters to TBA only', () => {
    const { display } = buildDeadlineSections(allDeadlines, { ...defaultOpts, filterStatus: 'tba' });
  ...
  it('splits TBA into exam / week-known / fully-unknown sub-groups when multiple exist', () => {
    const { sections } = buildDeadlineSections(allDeadlines, { ...defaultOpts, filterStatus: 'tba' });
  ...
  it('omits section labels when only one TBA sub-group is present', () => {
    const { sections } = buildDeadlineSections([examTba], { ...defaultOpts, filterStatus: 'tba' });
